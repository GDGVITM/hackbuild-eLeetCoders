...
export default function AuthPage() {
  ...
  const [organizationStatus, setOrganizationStatus] = useState<"checking" | "verified" | "not-verified" | null>(null)
  ...
  const checkOrganizationStatus = async (email: string) => {
    ...
    const organizationDomains = ["university.edu", "techclub.org", "studentassoc.edu", "campusorg.edu"]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    console.log("[v0] Form submitted:", { isLogin, email: userEmail })

    // Check if user came from organize event button
    const organizeIntent = searchParams.get("redirect")

    if (organizeIntent === "organizer") {
      console.log("[v0] Redirecting to organizer dashboard for demo")
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 transition-all duration-300">
            ...
            {searchParams.get("redirect") === "organizer" && (
              <div className="mt-4 p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border-2 border-purple-200">
                <div className="flex items-center justify-center mb-2">
                  ...
                  <h3 className="text-lg font-semibold text-purple-800">Organization Verification Required</h3>
                </div>
                <p className="text-sm text-purple-700 mb-3">
                  To access the Event Organizer Dashboard, you must be affiliated with a registered organization.
                </p>
                <div className="text-xs text-purple-600 bg-white/50 rounded-lg p-2">
                  <strong>Accepted domains:</strong> university.edu, techclub.org, studentassoc.edu, campusorg.edu
                </div>
              </div>
            )}
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {searchParams.get("redirect") === "organizer" && organizationStatus && (
                ...
                {organizationStatus === "verified" && (
                  <Alert className="border-green-200 bg-green-50">
                    ...
                    <AlertDescription className="text-green-700 font-medium">
                      ✅ Organization Verified! You can now access the Organizer Dashboard.
                    </AlertDescription>
                  </Alert>
                )}
                  ...
                  <Alert className="border-red-200 bg-red-50">
                    ...
                    <AlertDescription className="text-red-700">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm">
                          Your email domain is not associated with a registered organization. Only verified organization
                          members can create and manage events.
                        </p>
                        <p className="text-xs text-red-600">Contact support if you believe this is an error.</p>
                      </div>
                    </AlertDescription>
                  </Alert>
                )}
            ...
            <Button
              ...
              disabled={
                ...
                (searchParams.get("redirect") === "organizer" && organizationStatus === "not-verified")
              }
            >
              {organizationStatus === "checking" ? "Verifying..." : isLogin ? "Log In" : "Sign Up"}
            </Button>

            {searchParams.get("redirect") === "organizer" && !organizationStatus && userEmail && (
              <Button
                type="button"
                variant="outline"
